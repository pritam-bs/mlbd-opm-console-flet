from ....core.abstractions import BaseView
from ....presentation.features.home.view_model import HomeState, HomeViewModel, RequestType


class HomeIntent:
    def __init__(self):
        self._view_model = HomeViewModel()
        self._view_model.set_state_callback(self._on_state_received)

    @property
    def current_state(self):
        return self._view_model.current_state

    def bind(self, view: BaseView):
        self._view_model.state_driver.bind(view=view)

    async def get_bookings(self):
        await self._show_booking_request_loading()
        state = await self._view_model.get_booking()
        await self._view_model.state_driver.accept(new_state=state)

    def start_face_recognition(self):
        self._view_model.start_face_recognition()

    async def _on_state_received(self, state: HomeState):
        await self._view_model.state_driver.accept(new_state=state)

    def stop_face_recognition(self):
        self._view_model.stop_face_recognition()

    def start_synchronizer(self):
        self._view_model.start_synchronizers()

    def stop_model_synchronizer(self):
        self._view_model.stop_synchronizers()

    def start_booking_update_scheduler(self):
        self._view_model.start_booking_update_scheduler()

    def stop_booking_update_scheduler(self):
        self._view_model.stop_booking_update_scheduler()

    async def consume_breakfast(self):
        await self._show_meal_consume_request_loading()
        state = await self._view_model.consume_breakfast()
        await self._view_model.state_driver.accept(new_state=state)

    async def consume_lunch(self):
        await self._show_meal_consume_request_loading()
        state = await self._view_model.consume_lunch()
        await self._view_model.state_driver.accept(new_state=state)

    async def _show_meal_consume_request_loading(self):
        state = self._view_model.change_loading_state(
            is_loading=True, request=RequestType.meal_consume_request)
        await self._view_model.state_driver.accept(new_state=state)

    async def _hide_meal_consume_request_loading(self):
        state = self._view_model.change_loading_state(
            is_loading=False, request=RequestType.meal_consume_request)
        await self._view_model.state_driver.accept(new_state=state)

    async def _show_booking_request_loading(self):
        state = self._view_model.change_loading_state(
            is_loading=True, request=RequestType.booking_request)
        await self._view_model.state_driver.accept(new_state=state)

    async def _hide_booking_request_loading(self):
        state = self._view_model.change_loading_state(
            is_loading=False, request=RequestType.booking_request)
        await self._view_model.state_driver.accept(new_state=state)

    async def _clear_error(self, request: RequestType):
        state = self._view_model.clear_error(request=request)
        await self._view_model.state_driver.accept(new_state=state)
