...
class SplashIntent:
    ...
    async def change_user_id(self, user_id: str):
        state = self._view_model.validate_input(client_name=user_id)
        await self._view_model.state_driver.accept(new_state=state)

    async def change_password(self, password: str):
        state = self._view_model.validate_input(password=password)
        await self._view_model.state_driver.accept(new_state=state)

    async def submit(self):
        self.clear_error()
        self.show_loading()
        state = await self._view_model.submit()
        await self._view_model.state_driver.accept(new_state=state)
    ...
    async def clear_error(self):
        state = self._view_model.clear_error()
        await self._view_model.state_driver.accept(new_state=state)
